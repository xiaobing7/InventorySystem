...
class InventoryServicer(inventory_pb2_grpc.InventoryServiceServicer):
    ...
    def GetBook(self, request, context):
        for book in self.db:
            if book.title == request.title:
                return inventory_pb2.BookSearchResponse(book=book)
        return inventory_pb2.BookSearchResponse(None)
    ...
    def CreateBook(self, request, context):
        ...
        if book:
            ...
            return inventory_pb2.BookCreateResponse(message="Create book <%s> successfully"%book.title)
        else:
            return inventory_pb2.BookCreateResponse(message="Fail to create book, try agian")
def serve():
